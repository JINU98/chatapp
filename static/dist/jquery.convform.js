...
ConvState.prototype.printQuestion = function () {
  ...
  setTimeout(
    function () {
      ...
      if (
        ...
        if (this.next()) {
          setTimeout(
            function () {
              var messageObj = $(
                '<div class="message to typing lll"><div class="typing_loader"></div></div>'
              );
      ...
      if (
        this.current.input.hasOwnProperty("autoAnswer") &&
        this.current.input.autoAnswer === true &&
        this.current.next !== false
      ) {
        this.answerWith(
          this.current.input.answers[0].text,
          this.current.input.answers[0].value
        );
      }
...
ConvState.prototype.answerWith = function (answerText, answerObject) {
  ...
  if (this.current.input.element.value === "end") {
    return false;
  }
  var messageObj = $(
    '<div class="message to typing cc"><div class="typing_loader"></div></div>'
  );
  setTimeout(
    function () {
      $(this.wrapper).find("#messages").append(messageObj);
  ...
  $.fn.convform = function (options) {
    ...
    var inputs = $(this)
      ...
      .map(function () {
        ...
        if ($(this).attr("data-auto-answer")) {
          input["autoAnswer"] = true;
          input["answers"] = [
            {
              text: $(this).attr("data-auto-answer"),
              value: $(this).attr("data-auto-answer")
            }
          ];
        }
    ...
    if (inputs.length) {
      ...
      setTimeout(function () {
        $.when($("div.spinLoader").addClass("hidden")).done(function () {
          var messageObj = $(
            '<div class="message to typing gggg"><div class="typing_loader"></div></div>'
          );
